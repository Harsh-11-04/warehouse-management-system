      ...
      <div className="grid lg:grid-cols-3 gap-4">
        {isAdmin && (
          <div className="bg-white dark:bg-gray-800 rounded-xl border shadow-sm p-4 space-y-3">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Create Warehouse</h2>
            <div className="space-y-2 text-sm">
              ...
              <div>
                <label className="block mb-1 text-xs text-gray-500 dark:text-gray-400">Address</label>
                <InputText
                  value={address}
        ...
        <div className={`bg-white dark:bg-gray-800 rounded-xl border shadow-sm p-4 ${isAdmin ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
          <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Existing Warehouses</h2>
              ...
              <tbody>
                {warehouses.length ? (
                  warehouses.map((w: any) => (
                    <tr key={w._id} className="border-t border-gray-100 dark:border-gray-700">
                      <td className="p-2">{w.name}</td>
                      <td className="p-2 text-gray-500 dark:text-gray-400">{w.address}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={2} className="p-2 text-xs text-gray-400">
                      No warehouses yet.
                      {isAdmin && ' Create one using the form on the left.'}
                    </td>
                  </tr>
                )}
              </tbody>
